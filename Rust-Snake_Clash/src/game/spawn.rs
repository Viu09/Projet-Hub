...
use crate::constants::{ARENA_RADIUS, BOOST_ENERGY_MAX, DEMO_BOT_COUNT};
...
fn pick_spawn_pos(agents: &[Agent], arena_radius: f32) -> Vec2 {
    // Rejection sampling: avoid spawning too close to existing heads.
    for _ in 0..30 {
...
pub fn make_initial_agents() -> Vec<Agent> {
    ...
    // demo100: spectator-only (no player), lots of bots.
    // demo_play100: playable (player + lots of bots), reduced clutter.
    // normal: player + a handful of bots.
    ...
    if !spectator_demo {
        ...
        agents.push(Agent {
            ...
            color_head: YELLOW,
            color_body: ORANGE,
            snake: Snake::new_at(player_spawn, vec2(1.0, 0.0)),
            alive: true,
            respawn_left: 0.0,
            score: 0.0,
